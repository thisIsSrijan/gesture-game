...
while True:
    ...
    if result.multi_hand_landmarks:
        for handslms in result.multi_hand_landmarks:
            ...
            # Extracting relevant landmarks for thumbs up
            thumb_tip = landmarks[4]
            index_tip = landmarks[8]

            # Check if thumb is pointing towards the right
            thumb_is_right = thumb_tip[0] > index_tip[0]
            ...
            if classID == 0 and thumb_is_right and distance > 50:  
                className = "Right Pointing Thumb"
            else:
                className = "Unknown"

            hand_side = "Right" if landmarks[0][0] > x / 2 else "Left"

            text_position = (10, 50) if hand_side == "Right" else (x - 250, 50)
