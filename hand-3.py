...
while True:
    ...
    if result.multi_hand_landmarks:
        for handslms in result.multi_hand_landmarks:
            ...
            wrist = landmarks[0]
            ring_second = landmarks[15]

            # Check if thumb is pointing towards the right and other fingers are close to thumb (clenched fist)
            thumb_is_right = thumb_tip[0] > index_tip[0] and thumb_tip[0] > middle_tip[0] and thumb_tip[0] > ring_tip[0] and thumb_tip[0] > pinky_tip[0]

            # Calculate the distance between thumb tip and index finger tip
            distance = np.sqrt((thumb_tip[0] - index_tip[0])**2 + (thumb_tip[1] - index_tip[1])**2)

            clenched_fist = np.sqrt((ring_second[0] - wrist[0])**2 + (ring_second[1] - wrist[1])**2)

            # Predict gesture
            prediction = model.predict([landmarks])
            classID = np.argmax(prediction)

            if classID == 0 and thumb_is_right and distance > 50 and clenched_fist < 30:  
